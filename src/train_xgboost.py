...
FEATURES_FILENAME = 'xgboost_features.pkl' # File to save feature names
...
def train_forecasting_model(X_train: pd.DataFrame, y_train: pd.Series):
    ...
    model = XGBRegressor(objective='reg:squarederror', n_estimators=200, learning_rate=0.05, random_state=42, n_jobs=-1)
...
if __name__ == "__main__":
    ...
    if feature_df is not None:
        ...
        if X_train is not None and not X_train.empty:
             ...
             print("\nXGBoost model training script finished successfully!")
        else:
